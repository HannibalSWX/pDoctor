 ...
 * Description：图片异步下载类，包括图片的软应用缓存以及将图片存放到SDCard或者文件中
 * Author ： zhouqiang
 * Date ：2014-12-5 下午12:21:34
...
public class ImageDownloader {
	...
	public void imageDownload(String url, ImageView mImageView, String path,
			Context mContext, OnImageDownload download) {
		...
		String imageName = "";
		if (url != null) {
			imageName = UtilImageLoad.getInstance().getImageName(url);
		}
		// 先从软引用中拿数据
...
//			System.out.println("从软引用中拿数据--imageName==" + imageName);
			...
			// 文件中也没有，此时根据mImageView的tag，即url去判断该url对应的task是否已经在执行，如果在执行，本次操作不创建新的线程，否则创建新的线程。
				...
				MyAsyncTask task = new MyAsyncTask(url, mImageView, path,
						mContext, download);
				if (mImageView != null) {
					Log.i(TAG, "执行MyAsyncTask --> " + UtilImageLoad.flag);
					UtilImageLoad.flag++;
					task.execute();
					// 将对应的url对应的任务存起来
					map.put(url, task);
	 ...
	 * @return
	 */
	private Bitmap getBitmapFromFile(Context mContext, String imageName,
			String path) {
		Bitmap bitmap = null;
		if (imageName != null) {
			File file = null;
			String real_path = "";
			try {
				if (UtilImageLoad.getInstance().hasSDCard()) {
					real_path = UtilImageLoad.getInstance().getExtPath()
							+ (path != null && path.startsWith("/")
									? path
									: "/" + path);
				} else {
					real_path = UtilImageLoad.getInstance().getPackagePath(
							mContext)
							+ (path != null && path.startsWith("/")
									? path
									: "/" + path);
	 ...
	 * @return
	 */
	private boolean setBitmapToFile(String path, Context mContext,
		...
		try {
			if (UtilImageLoad.getInstance().hasSDCard()) {
				...
				real_path = UtilImageLoad.getInstance()
						.getPackagePath(mContext)
						+ (path != null && path.startsWith("/") ? path : "/"
								+ path);
			}
			file = new File(real_path, imageName);
	 ...
	 * @param imageName
	 */
	private void removeBitmapFromFile(String path, Context mContext,
			String imageName) {
		File file = null;
		String real_path = "";
		try {
		...
		protected Bitmap doInBackground(String... params) {
			...
			if (url != null) {
				try {
					...
					String imageName = UtilImageLoad.getInstance()
							.getImageName(url);
