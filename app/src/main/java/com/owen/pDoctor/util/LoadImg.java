...
public class LoadImg {
	...
	public Bitmap loadImage(final ImageView imageView, final String imageUrl,
		...
		// 图片保存到本地时的地址
		String filepath = Environment.getExternalStorageDirectory() + "/gsearch/" + filename;
		// System.out.println("ppppp"+filepath);
		// 查找一级缓存，看看是否有这张图片
		// 如果map当中有这个key返回一个true
		if (APP.getInstance().getImageCaches().containsKey(imageUrl)) {
			// System.out.println("一级已存在图片");
			// 找到对应图片软引用的封装
			SoftReference<Bitmap> soft = APP.getInstance()
					.getImageCaches().get(imageUrl);
			// 从软引用当中获取图片
			Bitmap bit = soft.get();
			if (bit != null)
				return bit;
		...
		if (fileUtiles.isBitmap(filename)) {
			// System.out.println("二级已存在图片");
			Bitmap bit = BitmapFactory.decodeFile(filepath);
			// 在二级缓存读取的时候直接添加到一级缓存当中
			APP.getInstance().getImageCaches()
					.put(imageUrl, new SoftReference<Bitmap>(bit));
			return bit;
		}
