...
public class ScaleImageFromSdcardActivity extends Activity implements OnScrollListener {
	Bitmap bm;
	...
	public void btnClick(View v) {
		...
		case R.id.btn_preview:
			if (imageAdapter.ClickList.size() > 0) {
				Intent intent = new Intent(ScaleImageFromSdcardActivity.this, PicturesShowActivity.class);
				Object[] data = imageAdapter.getClickList().toArray();
				intent.putExtra("selectPicture", data);
				ScaleImageFromSdcardActivity.this.startActivityForResult(intent, SHOW_SELECT_PICTURE);
				// 设置切换动画，从右边进入，左边退出
		...
		case R.id.btn_send_picture:
			...
			ScaleImageFromSdcardActivity.this.setResult(SHOW_PICTURE_RESULT, intent);
						...
						if (cursor != null) {
							if (counter > (IndexPposition - 1) * pageCount + i) {
								...
								if (bm != null) {
									...
									// Log.d("TAG", "bm" + bm);
									Bitmap btm2 = Bitmap.createScaledBitmap(bm, 150, 150, false); // 这里你可以自定义它的大小
									bitmapList.add(btm2);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == SHOW_SELECT_PICTURE && resultCode == SHOW_SELECT_PICTURE_RESULT) {
			// 获取传过来的选择的图片
