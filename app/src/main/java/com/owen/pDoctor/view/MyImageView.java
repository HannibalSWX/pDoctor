...
public class MyImageView extends ImageView {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (width > ImageShowerActivity.screenWidth) {
			// 如果图宽大于屏宽，就不用水平居中
			center(true, true);
		} else {
			center(true, true);
	...
	private void arithScaleRate() {
		float scaleWidth = ImageShowerActivity.screenWidth / (float) imageWidth;
		float scaleHeight = ImageShowerActivity.screenHeight
				/ (float) imageHeight;
	...
	public void setImageBitmap(Bitmap bitmap) {
		...
		arithScaleRate();
		// 缩放到屏幕大小
		zoomTo(scaleRate, ImageShowerActivity.screenWidth / 2f,
				ImageShowerActivity.screenHeight / 2f);
	...
	public void layoutToCenter() {
		...
		// 空白区域宽高
		float fill_width = ImageShowerActivity.screenWidth - width;
		float fill_height = ImageShowerActivity.screenHeight - height;
		...
		setImageMatrix(getImageViewMatrix());
		float ratio = ImageShowerActivity.screenWidth / (width * 1.0f);
		mSuppMatrix.postScale(ratio, ratio);
	}

	protected float getValue(Matrix matrix, int whichValue) {
		matrix.getValues(mMatrixValues);
		mMinZoom = (ImageShowerActivity.screenWidth / 2f) / imageWidth;
